...
class CartItemAPI(APIView):
    ...
    def get(self, request, id):
        try:
            cart = UserCart.objects.filter(id=id)
            cart_serializer = CartSerializer(cart, many=True)
            item_list = UserCartItem.objects.filter(cart_id=id)
            serializer = CartItemSerializer(item_list, many=True)
            return Response({"Message": "List of Cart Items", "data": {"cart_data":cart_serializer.data, "cart_items":{"cart_item_data":serializer.data}}, "status":200})
        except Exception as e:
            logging.error(e)
    ...
    def delete(self, request, pk):
        try:
            cart_item = UserCartItem.objects.get(id=pk)
            cart_item.delete()
